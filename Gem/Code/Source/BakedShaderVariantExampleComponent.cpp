...
namespace AtomSampleViewer
{
    ...
    namespace
    {
        ...
        namespace Products
        {
            ...
            //        AZStd::string::format("hotreloadtest_%s_%u.azshadervariant", apiName.GetCStr(), stableId.GetIndex());
            //    return filePath;
            //}

            // These materials are used to communicate which shader variant is being used, for screenshot comparison tests.
            static constexpr const char RootVariantIndicatorMaterialFilePath[] =
                "materials/hotreloadtest/testdata/variantselection_root.azmaterial";
            static constexpr const char FullyBakedVariantIndicatorMaterialFilePath[] =
                "materials/hotreloadtest/testdata/variantselection_fullybaked.azmaterial";
        } // namespace Products
    ...
    void BakedShaderVariantExampleComponent::InitTestDataFolders()
    {
    ...
    void BakedShaderVariantExampleComponent::Activate()
    {
        ...
        const Transform cameraTransform = Transform::CreateFromQuaternionAndTranslation(
            Quaternion::CreateFromAxisAngle(Vector3::CreateAxisZ(), AZ::Constants::Pi), Vector3{0.0f, 0.6f, 0.0f});
    ...
    void BakedShaderVariantExampleComponent::DeleteTestFile(const char* tempSourceFile)
    {
    ...
    void BakedShaderVariantExampleComponent::CopyTestFile(const AZStd::string& testDataFile, const AZStd::string& tempSourceFile)
    {
    ...
    const char* BakedShaderVariantExampleComponent::ToString(AzFramework::AssetSystem::AssetStatus status)
    {
        switch (status)
        {
    ...
    AzFramework::AssetSystem::AssetStatus BakedShaderVariantExampleComponent::GetTestAssetStatus(const char* tempSourceFile) const
    {
        ...
        AzFramework::AssetSystemRequestBus::BroadcastResult(
            status, &AzFramework::AssetSystem::AssetSystemRequests::GetAssetStatusSearchType, filePath.c_str(),
            AzFramework::AssetSystem::RequestAssetStatus::SearchType::Exact);

        return status;
    }

    void BakedShaderVariantExampleComponent::DrawAssetStatus(const char* tempSourceFile, bool includeFileName)
    {
    ...
    Data::AssetId BakedShaderVariantExampleComponent::GetAssetId(const char* productFilePath)
    {
        Data::AssetId assetId;
        Data::AssetCatalogRequestBus::BroadcastResult(
            assetId, &Data::AssetCatalogRequestBus::Events::GetAssetIdByPath, productFilePath, nullptr, false);
        return assetId;
    }

    void BakedShaderVariantExampleComponent::OnAssetReady(AZ::Data::Asset<AZ::Data::AssetData> asset)
    {
        if (m_initStatus == InitStatus::WaitingForDefaultMaterialToLoad && asset.GetId() == m_materialAsset.GetId())
        {
            ...
            else
            {
                ...
                m_meshChangedHandler =
                    AZ::Render::MeshFeatureProcessorInterface::ModelChangedEvent::Handler{[](AZ::Data::Instance<AZ::RPI::Model> model) {
                        ScriptRunnerRequestBus::Broadcast(&ScriptRunnerRequests::ResumeScript);
                    }};
    ...
    BakedShaderVariantExampleComponent::ShaderVariantStatus BakedShaderVariantExampleComponent::GetShaderVariantStatus() const
    {
    ...
    void BakedShaderVariantExampleComponent::OnTick([[maybe_unused]] float deltaTime, ScriptTimePoint /*scriptTime*/)
    {
        ...
        //    if (!materialAssetId.IsValid() && !materialTypeAssetId.IsValid() && !shaderAssetId.IsValid())
        //    {
        ...
        //    {
        //        m_meshFeatureProcessor->ReleaseMesh(m_shaderVariantIndicatorMeshHandle);
        //        m_shaderVariantIndicatorMeshHandle =
        //            m_meshFeatureProcessor->AcquireMesh(m_modelAsset, m_shaderVariantIndicatorMaterial_current);
    ...
    void BakedShaderVariantExampleComponent::ShowShadersComboBox()
    {
        if(!m_material)
            return;

        auto shaderCollection = m_material->GetShaderCollection();
        auto selectedShader = shaderCollection[m_selectedShaderIndex].GetShaderTag();

        if (ScriptableImGui::BeginCombo("Shaders", selectedShader.GetCStr(), ImGuiComboFlags_HeightLargest))
        {
            for (uint32_t i = 0; i < shaderCollection.size(); ++i)
            {
                AZStd::string shaderTag = shaderCollection[i].GetShaderTag().GetCStr();

                if(ScriptableImGui::Selectable(shaderTag.c_str()))
                {
