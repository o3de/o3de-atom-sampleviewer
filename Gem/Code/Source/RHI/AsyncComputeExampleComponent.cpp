...
namespace AtomSampleViewer
{
    ...
    void AsyncComputeExampleComponent::CreateQuad()
    {
        ...
        m_quadBufferPool->Init(RHI::AllDevices, bufferPoolDesc);
        ...
        AZ::RHI::StreamBufferView positionsBufferView = {
            *m_quadInputAssemblyBuffer, offsetof(BufferData, m_positions), sizeof(BufferData::m_positions), sizeof(VertexPosition)
        };

        AZ::RHI::StreamBufferView normalsBufferView = {
            *m_quadInputAssemblyBuffer, offsetof(BufferData, m_normals), sizeof(BufferData::m_normals), sizeof(VertexNormal)
        };

        AZ::RHI::StreamBufferView uvsBufferView = {
            *m_quadInputAssemblyBuffer, offsetof(BufferData, m_uvs), sizeof(BufferData::m_uvs), sizeof(VertexUV)
        };
    ...
    void AsyncComputeExampleComponent::CreateCopyTextureScope()
    {
        ...
        const auto executeFunction = [this](const RHI::FrameGraphExecuteContext& context, [[maybe_unused]] const ScopeData& scopeData)
        {
...

            {
                ...
                auto indexBufferView{m_quadIndexBufferView.GetDeviceIndexBufferView(context.GetDeviceIndex())};
                ...
                AZStd::vector<RHI::DeviceStreamBufferView> quadStreamBufferViews;
                for (const auto& view : m_quadStreamBufferViews[CopyTextureScope])
                    quadStreamBufferViews.emplace_back(view.GetDeviceStreamBufferView(context.GetDeviceIndex()));
                drawItem.m_streamBufferViews = quadStreamBufferViews.data();
        ...
        const auto executeFunction = [this](const RHI::FrameGraphExecuteContext& context, [[maybe_unused]] const ScopeData& scopeData)
        {
            ...
            for(uint32_t i = context.GetSubmitRange().m_startIndex; i < context.GetSubmitRange().m_endIndex; ++i)
            {
                if (i == 0)
                {
                    ...
                    AZStd::vector<AZ::RHI::DeviceStreamBufferView> quadStreamBufferViews;
                    for (const auto& view : m_quadStreamBufferViews[ShadowScope])
                        quadStreamBufferViews.emplace_back(view.GetDeviceStreamBufferView(context.GetDeviceIndex()));
                    drawItem.m_streamBufferViews = quadStreamBufferViews.data();
                ...
                else
                {
                    ...
                    for (const auto& mesh : m_model->GetLods()[0]->GetMeshes())
                    {
                        ...
                        drawItem.m_pipelineState =
                            m_modelPipelineStates[ShadowScope]->GetDevicePipelineState(context.GetDeviceIndex()).get();
                        ...
                        AZStd::vector<AZ::RHI::DeviceStreamBufferView> modelStreamBufferViews;
                        for (const auto& view : m_modelStreamBufferViews[ShadowScope])
                            modelStreamBufferViews.emplace_back(view.GetDeviceStreamBufferView(context.GetDeviceIndex()));
                        drawItem.m_streamBufferViews = modelStreamBufferViews.data();
        ...
        const auto executeFunction = [this](const RHI::FrameGraphExecuteContext& context, [[maybe_unused]] const ScopeData& scopeData)
        {
            ...
            for (uint32_t i = context.GetSubmitRange().m_startIndex; i < context.GetSubmitRange().m_endIndex; ++i)
            {
                if (i == 0)
                {
                    ...
                    drawItem.m_arguments = drawIndexed;
                    drawItem.m_pipelineState =
                        m_terrainPipelineStates[ForwardScope]->GetDevicePipelineState(context.GetDeviceIndex()).get();
                    auto quadIndexBufferView{ m_quadIndexBufferView.GetDeviceIndexBufferView(context.GetDeviceIndex()) };
                    ...
                    AZStd::vector<AZ::RHI::DeviceStreamBufferView> quadStreamBufferViews;
                    for (const auto& view : m_quadStreamBufferViews[ForwardScope])
                        quadStreamBufferViews.emplace_back(view.GetDeviceStreamBufferView(context.GetDeviceIndex()));
                    drawItem.m_streamBufferViews = quadStreamBufferViews.data();
                ...
                else
                {
                    ...
                    for (const auto& mesh : m_model->GetLods()[0]->GetMeshes())
                    {
                        ...
                        drawItem.m_arguments = mesh.m_drawArguments.GetDeviceDrawArguments(context.GetDeviceIndex());
                        drawItem.m_pipelineState =
                            m_modelPipelineStates[ForwardScope]->GetDevicePipelineState(context.GetDeviceIndex()).get();
                        auto indexBufferView{ mesh.m_indexBufferView.GetDeviceIndexBufferView(context.GetDeviceIndex()) };
                        drawItem.m_indexBufferView = &indexBufferView;
                        drawItem.m_shaderResourceGroupCount = static_cast<uint8_t>(RHI::ArraySize(shaderResourceGroups));
                        drawItem.m_shaderResourceGroups = shaderResourceGroups;
                        ...
                        AZStd::vector<AZ::RHI::DeviceStreamBufferView> modelStreamBufferViews;
                        for (const auto& view : m_modelStreamBufferViews[ForwardScope])
                            modelStreamBufferViews.emplace_back(view.GetDeviceStreamBufferView(context.GetDeviceIndex()));
                        drawItem.m_streamBufferViews = modelStreamBufferViews.data();
    ...
    void AsyncComputeExampleComponent::CreateTonemappingScope()
    {
        ...
        const auto executeFunction = [this](const RHI::FrameGraphExecuteContext& context, [[maybe_unused]] const ScopeData& scopeData)
        {
            ...
            decltype(dispatchItem.m_shaderResourceGroups) shaderResourceGroups = { { m_shaderResourceGroups[TonemappingScope][0]
                                                                                         ->GetRHIShaderResourceGroup()
                                                                                         ->GetDeviceShaderResourceGroup(
                                                                                             context.GetDeviceIndex())
                                                                                         .get() } };
    ...
    void AsyncComputeExampleComponent::CreateLuminanceMapScope()
    {
        ...
        const auto executeFunction = [this](const RHI::FrameGraphExecuteContext& context, [[maybe_unused]] const ScopeData& scopeData)
        {
...

            {
                ...
                AZStd::vector<AZ::RHI::DeviceStreamBufferView> quadStreamBufferViews;
                for (const auto& view : m_quadStreamBufferViews[LuminanceMapScope])
                    quadStreamBufferViews.emplace_back(view.GetDeviceStreamBufferView(context.GetDeviceIndex()));
                drawItem.m_streamBufferViews = quadStreamBufferViews.data();
                commandList->Submit(drawItem);
            }
    ...
    void AsyncComputeExampleComponent::CreateLuminanceReduceScopes()
    {
        ...
        for(uint32_t inputSize = AsyncCompute::s_luminanceMapSize, i = 0; inputSize > 1; ++i)
        {
            ...
            const auto compileFunction = [this, inputAttachmentId, outputAttachmentId, i](
                                             const RHI::FrameGraphCompileContext& context, [[maybe_unused]] const ScopeData& scopeData)
            {
            ...
            const auto executeFunction = [this, i, outputSize](const RHI::FrameGraphExecuteContext& context, [[maybe_unused]] const ScopeData& scopeData)
            {
                ...
                RHI::DeviceDispatchItem dispatchItem;
                decltype(dispatchItem.m_shaderResourceGroups) shaderResourceGroups = { { m_shaderResourceGroups[LuminanceReduceScope][i]
                                                                                             ->GetRHIShaderResourceGroup()
                                                                                             ->GetDeviceShaderResourceGroup(
                                                                                                 context.GetDeviceIndex())
                                                                                             .get() } };
