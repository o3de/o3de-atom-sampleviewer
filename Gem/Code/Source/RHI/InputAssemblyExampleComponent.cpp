...
namespace AtomSampleViewer
{
    ...
    void InputAssemblyExampleComponent::CreateComputeScope()
    {
        ...
        const auto compileFunction = [this](const RHI::FrameGraphCompileContext& context, const ScopeData& scopeData)
        {
            AZ_UNUSED(scopeData);
            {
                const RHI::BufferView* inputAssemblyBufferView =
                    context.GetBufferView(RHI::AttachmentId{ InputAssembly::InputAssemblyBufferAttachmentId });
            ...
            {
                const RHI::BufferView* inputAssemblyBufferView =
                    context.GetBufferView(RHI::AttachmentId{ InputAssembly::ImportedInputAssemblyBufferAttachmentId });
        ...
        const auto executeFunction = [this](const RHI::FrameGraphExecuteContext& context, const ScopeData& scopeData)
        {
            ...
            {
                dispatchItem.m_shaderResourceGroups[0] =
                    m_dispatchSRG[index]->GetRHIShaderResourceGroup()->GetDeviceShaderResourceGroup(context.GetDeviceIndex()).get();
    ...
    void InputAssemblyExampleComponent::CreateRasterScope()
    {
        ...
        const auto compileFunction = [this](const RHI::FrameGraphCompileContext& context, const ScopeData& scopeData)
        {
            AZ_UNUSED(scopeData);
            {
                ...
                if (inputAssemblyBufferView)
                {
                    ...
                    RHI::ValidateStreamBufferViews(
                        m_inputStreamLayout, AZStd::span<const RHI::StreamBufferView>(&m_streamBufferView[0], 1));
                }
            }

            {
                ...
                if (inputAssemblyBufferView)
                {
                    ...
                    RHI::ValidateStreamBufferViews(
                        m_inputStreamLayout, AZStd::span<const RHI::StreamBufferView>(&m_streamBufferView[1], 1));
                }
            }
        };
        ...
        const auto executeFunction = [this](const RHI::FrameGraphExecuteContext& context, const ScopeData& scopeData)
        {
            ...
            for (uint32_t index = context.GetSubmitRange().m_startIndex; index < context.GetSubmitRange().m_endIndex; ++index)
            {
                auto streamBufferView{ m_streamBufferView[index].GetDeviceStreamBufferView(context.GetDeviceIndex()) };
                drawItem.m_streamBufferViews = &streamBufferView;
