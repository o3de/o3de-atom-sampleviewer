    ...
    void RayTracingExampleComponent::CreateRayTracingAccelerationTableScope()
    {
        ...
        const auto executeFunction = [this]([[maybe_unused]] const RHI::FrameGraphExecuteContext& context, [[maybe_unused]] const ScopeData& scopeData)
        {
            ...
            commandList->BuildTopLevelAccelerationStructure(
                *m_rayTracingTlas->GetDeviceRayTracingTlas(RHI::MultiDevice::DefaultDeviceIndex), { m_triangleRayTracingBlas->GetDeviceRayTracingBlas(RHI::MultiDevice::DefaultDeviceIndex).get(), m_rectangleRayTracingBlas->GetDeviceRayTracingBlas(RHI::MultiDevice::DefaultDeviceIndex).get() });
        };
