...
namespace AtomSampleViewer
{
    ...
    void MeshExampleComponent::ActivateMultiMaterialDeferredPipeline()
    {
        ...
        auto* deferredMaterialFeatureProcessor =
            AZ::RPI::Scene::GetFeatureProcessorForEntityContextId<AZ::Render::DeferredMaterialFeatureProcessorInterface>(
                GetEntityContextId());

        AZ_Warning(
            "MeshExampleComponent", deferredMaterialFeatureProcessor && deferredMaterialFeatureProcessor->IsEnabled(),
            "MultiMaterial Deferred pipeline selected, but r_deferredRenderingEnabled=false. The pipeline will not get any deferred "
            "draw-items.");
